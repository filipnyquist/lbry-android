...
import com.fillerino.core.coins.DogecoinTest;
import com.fillerino.core.coins.NuBitsMain;
import com.fillerino.core.coins.Value;
import com.fillerino.core.coins.VpncoinMain;
import com.fillerino.core.exceptions.AddressMalformedException;
import com.fillerino.core.exceptions.Bip44KeyLookAheadExceededException;
import com.fillerino.core.exceptions.KeyIsEncryptedException;
import com.fillerino.core.exceptions.MissingPrivateKeyException;
import com.fillerino.core.network.AddressStatus;
import com.fillerino.core.network.BlockHeader;
import com.fillerino.core.network.interfaces.ConnectionEventListener;
import com.fillerino.core.network.interfaces.TransactionEventListener;
import com.fillerino.core.protos.Protos;
import com.fillerino.core.wallet.families.bitcoin.BitAddress;
import com.fillerino.core.wallet.families.bitcoin.BitBlockchainConnection;
import com.fillerino.core.wallet.families.bitcoin.BitSendRequest;
import com.fillerino.core.wallet.families.bitcoin.BitTransaction;
import com.fillerino.core.wallet.families.bitcoin.BitTransactionEventListener;
import com.fillerino.core.wallet.families.bitcoin.OutPointOutput;
import com.fillerino.core.wallet.families.bitcoin.TrimmedOutPoint;
...
public class WalletPocketHDTest {
    ...
    DeterministicKey masterKey = HDKeyDerivation.createMasterPrivateKey(Preconditions.checkNotNull(seed.getSeedBytes()));
...
//            } catch (Bip44KeyLookAheadExceededException e1) { /* ignore */ }
    ...
    private Sha256Hash send(Value value, WalletPocketHD w1, WalletPocketHD w2) throws Exception {
        Assert.assertEquals(w1.getCoinType(), w2.getCoinType());
    ...
    private void setTrustedTransaction(WalletPocketHD account, Sha256Hash txId) {
        BitTransaction tx = Preconditions.checkNotNull(account.getTransaction(txId));
    ...
    public void serializeUnencryptedNormal() throws Exception {
        ...
        Assert.assertEquals(pocket.getCoinType(), newPocket.getCoinType());
        ...
        for (BitTransaction tx : pocket.getTransactions().values()) {
            Assert.assertEquals(tx, newPocket.getTransaction(tx.getHash()));
            BitTransaction txNew = Preconditions.checkNotNull(newPocket.getTransaction(tx.getHash()));
        ...
        for (AddressStatus status : pocket.getAllAddressStatus()) {
            ...
            Assert.assertEquals(status, newPocket.getAddressStatus(status.getAddress()));
        }
...
//        pocket.broadcastTx(tx);
    ...
    private HashMap<AbstractAddress, ArrayList<ServerClient.HistoryTx>> getDummyHistoryTXs() throws AddressMalformedException, JSONException {
        HashMap<AbstractAddress, ArrayList<ServerClient.HistoryTx>> htxs = new HashMap<>(40);

        for (int i = 0; i < statuses.length; i++) {
            JSONArray jsonArray = new JSONArray(history[i]);
            ArrayList<ServerClient.HistoryTx> list = new ArrayList<>();
            for (int j = 0; j < jsonArray.length(); j++) {
                list.add(new ServerClient.HistoryTx(jsonArray.getJSONObject(j)));
            }
    ...
    private HashMap<AbstractAddress, ArrayList<ServerClient.UnspentTx>> getDummyUnspentTXs() throws AddressMalformedException, JSONException {
        HashMap<AbstractAddress, ArrayList<ServerClient.UnspentTx>> utxs = new HashMap<>(40);

        for (int i = 0; i < statuses.length; i++) {
            JSONArray jsonArray = new JSONArray(unspent[i]);
            ArrayList<ServerClient.UnspentTx> list = new ArrayList<>();
            for (int j = 0; j < jsonArray.length(); j++) {
                list.add(new ServerClient.UnspentTx(jsonArray.getJSONObject(j)));
            }
    ...
    class MockBlockchainConnection implements BitBlockchainConnection {
        ...
        final HashMap<AbstractAddress, ArrayList<ServerClient.HistoryTx>> historyTxs;
        final HashMap<AbstractAddress, ArrayList<ServerClient.UnspentTx>> unspentTxs;
        ...
        public void getUnspentTx(AddressStatus status, BitTransactionEventListener listener) {
            List<ServerClient.UnspentTx> utx = unspentTxs.get(status.getAddress());
            if (utx == null) {
