...
public class TrimmedTransaction extends Transaction {
    ...
    public TrimmedTransaction(NetworkParameters params, Sha256Hash hash, int numberOfOutputs) {
        super(params);
        Preconditions.checkState(numberOfOutputs >= 0, "Number of outputs cannot be negative");
        this.hash = hash;
    ...
    public TransactionOutput addOutput(int index, TransactionOutput to) {
        Preconditions.checkArgument(index >= 0 && index < numberOfOutputs, "Index out of range");
        Preconditions.checkNotNull(trimmedOutputs, "Cannot add more outputs")
                .put(index, new TrimmedOutput(to, index, this));
        return to;
    }

    public void addAllOutputs(List<TransactionOutput> outputs) {
        Preconditions.checkArgument(outputs.size() == numberOfOutputs, "Number of outputs don't match");
