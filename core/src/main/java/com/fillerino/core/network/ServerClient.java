...
import com.fillerino.stratumj.StratumClient;
import com.fillerino.stratumj.messages.CallMessage;
import com.fillerino.stratumj.messages.ResultMessage;
...
//        connectionExec.setRemoveOnCancelPolicy(true);
    ...
    private StratumClient createStratumClient() {
        Preconditions.checkState(stratumClient == null);
...
//    }
    ...
    public void subscribeToBlockchain(final TransactionEventListener listener) {
        Preconditions.checkNotNull(stratumClient);
    ...
    private File getTxCacheFile(Sha256Hash txHash) {
        return new File(new File(Preconditions.checkNotNull(cacheDir), type.getId()), txHash.toString());
    }
        ...
        Futures.addCallback(result, new FutureCallback<ResultMessage>() {
            ...
            public void onSuccess(ResultMessage result) {
                try {
                    ...
                    Preconditions.checkState(tx.getHash().toString().equals(txId));
                ...
                if (log.isDebugEnabled()) {
                    try {
                        log.debug("Server {} version {} OK", type.getName(),
                                Preconditions.checkNotNull(result).getResult().get(0));
