...
public class SendOutput extends LinearLayout {
    ...
    public SendOutput(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                com.fillerino.wallet.R.styleable.SendOutput, 0, 0);
        try {
            setIsFee(a.getBoolean(com.fillerino.wallet.R.styleable.SendOutput_is_fee, false));
        } finally {
            a.recycle();
        }
    }
    ...
    private void inflateView(Context context) {
        LayoutInflater.from(context).inflate(com.fillerino.wallet.R.layout.transaction_output, this, true);

        sendTypeText = (TextView) findViewById(com.fillerino.wallet.R.id.send_output_type_text);
        amount = (TextView) findViewById(com.fillerino.wallet.R.id.amount);
        symbol = (TextView) findViewById(com.fillerino.wallet.R.id.symbol);
        amountLocal = (TextView) findViewById(com.fillerino.wallet.R.id.local_amount);
        symbolLocal = (TextView) findViewById(com.fillerino.wallet.R.id.local_symbol);
        addressLabelView = (TextView) findViewById(com.fillerino.wallet.R.id.output_label);
        addressView = (TextView) findViewById(com.fillerino.wallet.R.id.output_address);
    ...
    private String getSendLabel() {
        if (sendLabel == null) {
            return getResources().getString(com.fillerino.wallet.R.string.send);
        } else {
            return sendLabel;
        }
    }
    ...
    private String getReceiveLabel() {
        if (receiveLabel == null) {
            return getResources().getString(com.fillerino.wallet.R.string.receive);
        } else {
            return receiveLabel;
        }
    }
    ...
    private String getFeeLabel() {
        if (feeLabel == null) {
            return getResources().getString(com.fillerino.wallet.R.string.fee);
        } else {
            return feeLabel;
        }
    }
