...
import com.fillerino.core.wallet.SignedMessage;
...
import com.fillerino.wallet.tasks.SignVerifyMessageTask;
...
public class SignVerifyMessageFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (args != null) {
            if (args.containsKey(Constants.ARG_ACCOUNT_ID)) {
                ...
                account = Preconditions.checkNotNull(application.getAccount(accountId));
            }
            Preconditions.checkNotNull(account, "No account selected");
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(com.fillerino.wallet.R.layout.fragment_sign_message, container, false);

        signingAddressView = (AutoCompleteTextView) view.findViewById(com.fillerino.wallet.R.id.signing_address);
        ArrayAdapter<AbstractAddress> adapter = new ArrayAdapter<>(getActivity(), com.fillerino.wallet.R.layout.item_simple,
                account.getActiveAddresses());
        signingAddressView.setAdapter(adapter);

        messageView = (EditText) view.findViewById(com.fillerino.wallet.R.id.message);
        signatureView = (EditText) view.findViewById(com.fillerino.wallet.R.id.signature);

        addressError = (TextView) view.findViewById(com.fillerino.wallet.R.id.address_error_message);
        signatureOK = (TextView) view.findViewById(com.fillerino.wallet.R.id.signature_ok);
        signatureError = (TextView) view.findViewById(com.fillerino.wallet.R.id.signature_error);

        verifyButton = (Button) view.findViewById(com.fillerino.wallet.R.id.button_verify);
        ...
        signButton = (Button) view.findViewById(com.fillerino.wallet.R.id.button_sign);
    ...
    private void showSignVerifyStatus(SignedMessage signedMessage) {
        ...
        switch (signedMessage.getStatus()) {
            case SignedOK:
                ...
                signatureOK.setText(com.fillerino.wallet.R.string.message_signed);
                break;
            case VerifiedOK:
                signatureOK.setVisibility(View.VISIBLE);
                signatureOK.setText(com.fillerino.wallet.R.string.message_verified);
                break;
            default:
    ...
    private void showSignVerifyError(SignedMessage.Status status) {
        ...
        switch (status) {
            case AddressMalformed:
                ...
                addressError.setText(com.fillerino.wallet.R.string.address_error);
                break;
            case KeyIsEncrypted:
                addressError.setVisibility(View.VISIBLE);
                addressError.setText(com.fillerino.wallet.R.string.wallet_locked_message);
                break;
            case MissingPrivateKey:
                addressError.setVisibility(View.VISIBLE);
                addressError.setText(com.fillerino.wallet.R.string.address_not_found_error);
                break;
            case InvalidSigningAddress:
            case InvalidMessageSignature:
                signatureError.setVisibility(View.VISIBLE);
                signatureError.setText(com.fillerino.wallet.R.string.message_verification_failed);
                break;
            case Unknown:
            default:
                signatureError.setVisibility(View.VISIBLE);
                signatureError.setText(com.fillerino.wallet.R.string.error_generic);
        }
    }
