...
import com.fillerino.core.network.ConnectivityHelper;
import com.fillerino.core.network.ServerClients;
import com.fillerino.core.wallet.BitWalletSingleKey;
...
public class SweepWalletFragment extends Fragment {
    ...
    @Bind(com.fillerino.wallet.R.id.private_key_input) View privateKeyInputView;
    @Bind(com.fillerino.wallet.R.id.sweep_wallet_key) EditText privateKeyText;
    @Bind(com.fillerino.wallet.R.id.passwordView) View passwordView;
    @Bind(com.fillerino.wallet.R.id.sweep_error) TextView errorΜessage;
    @Bind(com.fillerino.wallet.R.id.passwordInput) EditText password;
    @Bind(com.fillerino.wallet.R.id.sweep_loading) View sweepLoadingView;
    @Bind(com.fillerino.wallet.R.id.sweeping_status) TextView sweepStatus;
    @Bind(com.fillerino.wallet.R.id.button_next) Button nextButton;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(com.fillerino.wallet.R.layout.fragment_sweep, container, false);
    ...
    @OnClick(com.fillerino.wallet.R.id.scan_qr_code)
    void handleScan() {
        startActivityForResult(new Intent(getActivity(), ScanActivity.class), REQUEST_CODE_SCAN);
    }

    @OnClick(com.fillerino.wallet.R.id.button_next)
    ...
    @OnFocusChange(com.fillerino.wallet.R.id.sweep_wallet_key)
    void onPrivateKeyInputFocusChange(final boolean hasFocus) {
        if (!hasFocus) validatePrivateKey();
    }

    @OnTextChanged(value = com.fillerino.wallet.R.id.sweep_wallet_key, callback = AFTER_TEXT_CHANGED)
    ...
    private void updateErrorView() {
        switch (error) {
            case NONE:
                ...
                break;
            case BAD_FORMAT:
                errorΜessage.setText(com.fillerino.wallet.R.string.sweep_wallet_bad_format);
                setVisible(errorΜessage);
                break;
            case BAD_COIN_TYPE:
                errorΜessage.setText(com.fillerino.wallet.R.string.sweep_wallet_bad_coin_type);
                setVisible(errorΜessage);
                break;
            case BAD_PASSWORD:
                errorΜessage.setText(com.fillerino.wallet.R.string.sweep_wallet_bad_password);
                setVisible(errorΜessage);
                break;
            case ZERO_COINS:
                errorΜessage.setText(com.fillerino.wallet.R.string.sweep_wallet_zero_coins);
                setVisible(errorΜessage);
                break;
            case NO_CONNECTION:
                errorΜessage.setText(com.fillerino.wallet.R.string.disconnected_label);
                setVisible(errorΜessage);
                break;
            case GENERIC_ERROR:
                errorΜessage.setText(com.fillerino.wallet.R.string.error_generic);
                setVisible(errorΜessage);
                break;
        }
    }
    ...
    private void updateStatusView() {
        ...
        switch (status) {
            case DECODING:
                sweepStatus.setText(com.fillerino.wallet.R.string.sweep_wallet_key_decoding);
                break;
            case LOADING:
                sweepStatus.setText(com.fillerino.wallet.R.string.sweep_wallet_key_loading);
                break;
            case SIGNING:
                sweepStatus.setText(com.fillerino.wallet.R.string.sweep_wallet_key_signing);
                break;
        }
    ...
    private void onTransactionPrepared(SendRequest request) {
        ...
        if (listener != null) {
            listener.onSendTransaction(Preconditions.checkNotNull(request));
        }
    }
