...
public class DebuggingFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(com.fillerino.wallet.R.layout.fragment_debugging, container, false);
    ...
    @OnClick(com.fillerino.wallet.R.id.button_execute_password_test)
    void onExecutePasswordTest() {
        if (wallet.isEncrypted()) {
            showUnlockDialog();
        } else {
            DialogBuilder.warn(getActivity(), com.fillerino.wallet.R.string.wallet_is_not_locked_message)
    ...
    private class PasswordTestTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            Dialogs.ProgressDialogFragment.show(getFragmentManager(),
                    getString(com.fillerino.wallet.R.string.seed_working), PROCESSING_DIALOG_TAG);
        }
        ...
        private void tryDecrypt(DeterministicKey masterKey, CharSequence password, UnlockResult result) {
            KeyCrypter crypter = Preconditions.checkNotNull(masterKey.getKeyCrypter());
